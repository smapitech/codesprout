...
class AssignmentController extends Controller
{
    ...
    public function index(AssignmentReportService $reportService): Response
    {
        ...
        $assignments = Assignment::query()
            ...
            ->where(function ($query) use ($teacher): void {
                $query->where('owner_id', $teacher->id)
                    ...
                    ->orWhere('status', ContentStatus::Published);
            })
        ...
        return Inertia::render('assignments/index', [
            ...
            'summary' => $reportService->teacherSummary($teacher),
            'markingQueue' => $reportService->teacherMarkingQueue($teacher),
    ...
    public function create(): Response
    {
        ...
        return Inertia::render('assignments/builder', [
            ...
            'assignment' => null,
            'version' => null,
            'action' => route('teacher.assignments.store', absolute: false),
            'method' => 'post',
            'publishAction' => null,
            'allocateAction' => null,
            ...
            'builderOptions' => $this->builderOptions(request()->user()),
    ...
    public function store(AssignmentVersionRequest $request, AssignmentVersionService $versionService): RedirectResponse
    {
        ...
        return to_route('teacher.assignments.show', $version->assignment)->with('status', 'Assignment draft created.');
    }
    ...
    public function show(Assignment $assignment, AssignmentPublicationService $publicationService, AssignmentQuestionHandlerRegistry $registry): Response
    {
        ...
        return Inertia::render('assignments/builder', [
            ...
            'action' => route('teacher.assignments.update', $assignment, absolute: false),
            'method' => 'put',
            'publishAction' => route('teacher.assignments.publish', $assignment, absolute: false),
            'allocateAction' => route('teacher.assignments.allocate', $assignment, absolute: false),
            'submitLabel' => 'Save draft',
    ...
    public function update(AssignmentVersionRequest $request, Assignment $assignment, AssignmentVersionService $versionService): RedirectResponse
    {
        ...
        return to_route('teacher.assignments.show', $assignment)->with('status', 'Assignment draft saved.');
    }
    ...
    public function publish(Assignment $assignment, AssignmentPublicationService $publicationService): RedirectResponse
    {
        ...
        return to_route('teacher.assignments.show', $assignment)->with('status', 'Assignment published.');
    }
    ...
    public function allocate(Assignment $assignment, AssignmentAllocationRequest $request, AssignmentAllocationService $allocationService): RedirectResponse
    {
        ...
        return to_route('teacher.assignments.show', $assignment)->with('status', 'Assignment allocated.');
    }

    private function builderOptions(?User $teacher): array
    {
        abort_unless($teacher instanceof User, 403);

        $classes = $teacher->teachingClasses()
            ->orderBy('name')
            ->get(['classes.id', 'classes.name', 'classes.class_code', 'classes.curriculum_world_id']);
        $classIds = $classes->pluck('id')->all();
        $worldIds = $classes->pluck('curriculum_world_id')->filter()->unique()->all();

        return [
            'classes' => $classes->map(static fn ($class): array => [
                'id' => $class->id,
                'name' => $class->name,
            ...
            'children' => User::role('child')
                ->whereHas('enrolledClasses', fn ($query) => $query->whereIn('classes.id', $classIds))
                ->with('childProfile')
                ->orderBy('name')
                ->get(['id', 'name'])
